# License and Attribution

## Main License

**Bridge-MCP** is licensed under the [Apache License, Version 2.0](LICENSE).

Copyright 2026 Sorata

You may obtain a copy of the License at:
- http://www.apache.org/licenses/LICENSE-2.0
- [LICENSE](LICENSE) file in this repository

## Required Attribution

When distributing this software or derivative works, you **must**:

1. **Include the LICENSE file** from this repository
2. **Include the NOTICE file** (if present)
3. **State changes made** to the original code (if modified)
4. **Retain all copyright notices** in source files

## Example Attribution

### In Documentation
```
This software uses Bridge-MCP (https://github.com/SorataYang/qiao-mcp)
Copyright 2026 Sorata
Licensed under the Apache License, Version 2.0
```

### In Binary Distributions
```
Bridge-MCP - MCP server for intelligent bridge structural design
Copyright 2026 Sorata
Licensed under the Apache License, Version 2.0
See LICENSE file for details
```

### In Source Code
```python
# Bridge-MCP - MCP server for intelligent bridge structural design
# Copyright 2026 Sorata (https://github.com/SorataYang/qiao-mcp)
# Licensed under the Apache License, Version 2.0
```

## Third-Party Licenses

See [NOTICE](NOTICE) for a complete list of third-party dependencies and their licenses.

## Questions?

For licensing questions, contact the project maintainer or open an issue at:
https://github.com/SorataYang/qiao-mcp/issues
